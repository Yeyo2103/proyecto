...
$(function () {
    ...
    $('.btnCashRegister').on('click', function () {
        execute_ajax_request({
            'params': {
                'action': 'search_cash_register'
            },
            'success': function (request) {
                $('input[name="amount"]').val(request.amount);
            }
        });
        $('#myModalCashRegister').modal('show');
    });

    $('.btnSaveCashRegister').on('click', function () {
        var params = new FormData();
        params.append('action', 'create_cash_register');
        params.append('amount', $('input[name="amount"]').val());
        var args = {
            'params': params,
            'content': '¿Estas seguro de abrir la caja?',
            'success': function (request) {
                alert_sweetalert({
                    'message': 'Se ha creado abierto correctamente la caja',
                    'timer': 3000,
                    'callback': function () {
                        location.reload();
                    }

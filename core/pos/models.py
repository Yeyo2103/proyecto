...
class CashRegister(models.Model):
    ...
    initial_amount = models.DecimalField(max_digits=9, decimal_places=2, default=0.00, verbose_name='Saldo inicial')
    final_amount = models.DecimalField(max_digits=9, decimal_places=2, default=0.00)
    ...
    def current_amount(self):
        if self.active:
            sales = float(Sale.objects.filter(date_joined=self.opening_date, payment_type=PAYMENT_TYPE[0][0]).aggregate(result=Coalesce(Sum('total'), 0.00, output_field=FloatField()))['result'])
            expenses = float(Expenses.objects.filter(date_joined=self.opening_date).aggregate(result=Coalesce(Sum('valor'), 0.00, output_field=FloatField()))['result'])
            return float(self.initial_amount) + sales - expenses
        return 0.00

    def toJSON(self):
        item = model_to_dict(self)
        item['current_amount'] = self.current_amount
        item['user'] = self.user.toJSON()
        item['initial_amount'] = float(self.initial_amount)
        item['final_amount'] = float(self.final_amount) if self.final_amount else '---'
        item['opening_date'] = self.opening_date.astimezone().strftime('%Y-%m-%d %H:%M:%S')
        item['closing_date'] = self.closing_date.astimezone().strftime('%Y-%m-%d %H:%M:%S') if self.closing_date else '---'
        return item

    class Meta:
        verbose_name = 'Caja registradora'
        verbose_name_plural = 'Cajas registradora'
        default_permissions = ()
        permissions = (
            ('view_cash_register', 'Can view Caja registradora'),
            ('add_cash_register', 'Can add Caja registradora'),
            ('delete_cash_register', 'Can delete Caja registradora'),
        )

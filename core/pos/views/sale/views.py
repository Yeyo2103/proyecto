...
from core.pos.mixins import ValidateCashRegisterMixin
...
class SaleListView(GroupPermissionMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            elif action == 'create_cash_register':
                with transaction.atomic():
                    CashRegister.objects.filter(user_id=request.user.id, active=True).update(active=False)
                    cash_register = CashRegister()
                    cash_register.user = request.user
                    cash_register.initial_amount = float(request.POST['amount'])
                    cash_register.save()
            elif action == 'search_cash_register':
                data['amount'] = 0.00
                cash_register = CashRegister.objects.filter(user_id=request.user.id, active=True).first()
                if cash_register:
                    data['amount'] = cash_register.current_amount
            else:
...
class SaleCreateView(GroupPermissionMixin, ValidateCashRegisterMixin, CreateView):
    model = Sale

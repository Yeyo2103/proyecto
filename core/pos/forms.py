...
class CashRegisterForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['user'].queryset = User.objects.filter(is_active=True).exclude(groups__id=settings.GROUPS['client'])
        self.fields['initial_amount'].widget.attrs['autofocus'] = True
    ...
    class Meta:
        ...
        widgets = {
            ...
            'opening_date': forms.DateInput(format='%Y-%m-%d', attrs={
                'class': 'form-control datetimepicker-input',
                'id': 'date_joined',
                'value': datetime.now().strftime('%Y-%m-%d'),
                'data-toggle': 'datetimepicker',
        ...
        exclude = ['closing_date', 'final_amount', 'active']
